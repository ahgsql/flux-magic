...
import { magicPrompt } from './prompts.js';
...
import { findNearestAspectRatio } from './helpers.js';
dotenv.config();
...
app.post('/generate', async (req, res) => {
    try {
        ...
        if (process.env.LLM == "ANTHROPIC") {
            finalPrompt = await anthropicChat(aiPrompt, "sonnet");
            finalPrompt = JSON.parse(finalPrompt.content).enhancedPrompt

        } else {
            finalPrompt = await ollamaChat(aiPrompt);
            finalPrompt = JSON.parse(finalPrompt.content).enhancedPrompt
        }
